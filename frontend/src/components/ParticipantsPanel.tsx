...
export function ParticipantsPanel({ participants, onClose }: ParticipantsPanelProps) {
  ...
  const renderRow = (p: Participant) => (
    <div key={p.uid} className="flex items-center gap-3 px-4 py-2.5 transition" onMouseEnter={e => (e.currentTarget.style.background = "color-mix(in srgb, var(--primary) 8%, transparent)")} onMouseLeave={e => (e.currentTarget.style.background = "")}>
      ...
      <span className="flex-1 text-sm truncate" style={{ color: "var(--text)" }}>{p.name}</span>
      {p.role === "creator" ? (
        <span className="flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ color: "var(--creator-text)", background: "var(--creator-bg)" }}>
        ...
        <span className="flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-full" style={{ color: "var(--text-faint)", background: "var(--surface-muted)" }}>
          <User size={11} /> Member
        </span>
      )}
    </div>
  ...
  return (
    <div className="absolute inset-y-0 right-0 w-64 border-l flex flex-col z-30 shadow-xl" style={{ background: "var(--sidebar-item)", borderColor: "color-mix(in srgb, var(--primary) 15%, transparent)" }}>
      <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "color-mix(in srgb, var(--primary) 15%, transparent)" }}>
        <span className="text-sm font-semibold" style={{ color: "var(--text)" }}>Members · {participants.length}</span>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {admins.length > 0 && (
          <div>
            <p className="px-4 pt-2 pb-1 text-[11px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-faint)" }}>Admins — {admins.length}</p>
            {admins.map(renderRow)}
          </div>
        )}
        {members.length > 0 && (
          <div>
            <p className="px-4 pt-3 pb-1 text-[11px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-faint)" }}>Members — {members.length}</p>
